...
export default function TechItemCard(props: ItemProps): ReactElement {
  ...
  const chip = (
    <Chip
      variant="light"
      startContent={<span className="ml-2 font-bold">TL</span>}
    >
      <span className="font-bold">{props.item.techLevel}</span>
    </Chip>
  )
  return (
    <ShopItemCard
      {...props}
      headerStyle={techLevelHeaderStyles[props.item.techLevel]}
      cardStyle={techLevelCardStyles[props.item.techLevel]}
      chips={[chip, ...(props.chips ?? [])]}
    />
  )
}
